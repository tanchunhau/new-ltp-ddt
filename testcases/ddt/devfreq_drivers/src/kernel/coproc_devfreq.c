...
static int coproc_set_target(struct device *dev, unsigned long *_freq, u32 flags)
{
...
static int coproc_devfreq_probe(struct platform_device *pdev) {
  ...
  if (!node_ptr) {
    dev_err(dev, "%s: Could not read clock-initial-frequency from device tree \n", __func__);
    goto out;
  }
  ...
  d->devfreq = devfreq_add_device(dev, &d->profile, "userspace",
          NULL);
  if (IS_ERR(d->devfreq)) {
  ...
  if (IS_ERR(clk_nb)) {
    ...
    if (err == -EPROBE_DEFER) {
      ...
      dev_err(dev,
        "Failed to register coproc0 clock notifier: %d\n",
        err);
    }
    goto out_remove;
  }
...
MODULE_AUTHOR("Carlos Hernandez <ceh@ti.com>, "
