...
static ssize_t get_regbase(struct device *dev,
...
static struct device_attribute iommu_test_attr =
	__ATTR(regbase,  S_IRUGO, get_regbase,  NULL);
...
static int __init iommu_test_init(void)
{
    ...
    //added to get iommu base register adress
    pdev = to_platform_device(dev);

    retval = device_create_file(dev, &iommu_test_attr);
    if (retval)
		dev_info(dev, "Attribut creation failed...\n");

    res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
    iommu_regbase = res->start; 
    if (res)
        pr_info("res start = 0x%x size = 0x%x\n",iommu_regbase, resource_size(res));
